...
public class MessageAdapter extends SimpleAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.identificationNo.setText(msgList.get(position).get("identificationNo"));
		
		holder.fullName.setTag(position);
		holder.fullName.setText(Integer.toString(position+1));
		final String test=msgList.get(position).get(
				"message");
		...
		holder.message.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Message msgToActivity = MainActivity.mUiHandler.obtainMessage(Constant.MESSAGE,test);
				msgToActivity.arg1=Integer.parseInt((String) holder._id.getText());
		...
		holder.signedReceiver.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Message msgToActivity = MainActivity.mUiHandler.obtainMessage(Constant.STOPINFO,test);
