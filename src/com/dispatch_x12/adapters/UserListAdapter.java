	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			JSONObject rowInfo = (JSONObject) JsonHelper.toJSON(mList
					.get(position));
			loadInfo = rowInfo.getString("firstName") + " "
					+ rowInfo.optString("lastName");
			JSONArray jTest = rowInfo.optJSONArray("payrollInfo");
			if (jTest != null) {
				address = rowInfo.optJSONArray("payrollInfo").optJSONObject(0)
						.optString("payrollInfo");
			} else {
				address = rowInfo.getString("_id");
			}
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Message msgToActivity = MainActivity.mUiHandler
						.obtainMessage(Constant.EMPLOYEEDATA);
				JSONObject rowInfo;
				try {
					rowInfo = (JSONObject) JsonHelper.toJSON(mList.get(row));
		...
		//
		// // Trimming the string is just for
