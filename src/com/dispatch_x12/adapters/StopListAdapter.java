	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.geoCode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// int lonlat[]=MessageProcessing.searchAddress(mContext,
				// address);
				...
				if (addressList != null && addressList.size() > 0) {
					...
					toDisplay = Integer.toString(row) + "|"
							+ Double.toString(addressFound.getLatitude()) + "|"
							+ Double.toString(addressFound.getLongitude());
		...
		holder.identificationNo.setText(mStopList.get(position).get(
				"identificationNo"));

		holder.stopNo.setText(Integer.toString(position + 1));
		final String loadInfo = mStopList.get(position).get("message");
		...
		// MessageProcessing.verifyAddress().execute(params).get();
