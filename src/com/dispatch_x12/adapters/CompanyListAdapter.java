	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			JSONObject rowInfo = (JSONObject) JsonHelper.toJSON(mList
					.get(position));
			loadInfo = rowInfo.getString("companyName") + " "
					+ rowInfo.optString("isaIdQualifier") + "-"
					+ rowInfo.optString("isaId");

			if (rowInfo.optJSONArray("Address") != null) {
				JSONArray jArray = (JSONArray) JsonHelper.toJSON(rowInfo
						.optJSONArray("Address"));
				// Will actually want to loop thru the address array and get
				// type of address i want....like shipto,consignee,
